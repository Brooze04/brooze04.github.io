...
const threatFunctions = {
	sourceThreatenTarget(ev, fight, amount, useThreatCoeffs = true, extraCoeff = 1) { // extraCoeff is only used for tooltip text
		...
		if (!a || !b) return;
		let coeff = (useThreatCoeffs ? a.threatCoeff(ev.ability.type) : 1) * extraCoeff;
	...
	unitThreatenEnemiesSplit(ev, unit, fight, amount, useThreatCoeffs = true) {
		...
		if (!u) return;
		let coeff = useThreatCoeffs ? u.threatCoeff(ev.ability.type) : 1;
...
22289: handler_bossDropThreatOnDebuff(0), // Brood Power: Green
